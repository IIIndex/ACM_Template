...
struct Node{
    ...
    int id;                                     //输入数据的标识
    int way;                                    //超平面的维度号
    bool operator < (const Node &oth) const{    //根据超平面维度决定比较函数
...
ll dis_sqr(ll a, ll b){                         //距离平方
    return (a - b) * (a - b);
}

double var[K];

void build(int l, int r, int rt, int dept){         //注意l要从0开始
    if(l > r)return;
    flag[rt] = 1;                                   //当前节点存在
    int lc = rt << 1;
    int rc = rt << 1 | 1;
...
void query(Node &nd, int m, int rt, int dept){      //查询离nd点前m小的点
    if(flag[rt] == -1)return;                       //节点不存在
    ...
    int lc = rt << 1;
    int rc = rt << 1 | 1;
    if(nd.a[dim] >= pt[rt].a[dim]) swap(lc, rc);    //nd在当前分维上的大小大于等于rt在当前分维上的大小
    ...
    if (pq.size() < m) { pq.push(cur); fg = 1; }       //队列未满
        ...
        if(dis_sqr(nd.a[dim], pt[rt].a[dim]) < pq.top().first){ //右儿子内可能存在更优点
            fg = 1;
        }
    }
    if(~flag[rc] && fg){                            //访问右儿子
